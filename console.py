...
class HBNBCommand(cmd.Cmd):
    ...
    def do_create(self, args):
        ...
        name_of_class = argument_array[0]
        if name_of_class not in HBNBCommand.classes:
        ...
        for parameter_indx in range(1, len(argument_array)):
            ...
            if len(parameter_arr) == 2:
                ky = parameter_arr[0]
                if ky not in HBNBCommand.my_keys[name_of_class]:
                    continue
                val = self.parse_value(parameter_arr[1])
                if val is not None:
                    setattr(new_instance, ky, val)
            else:
                pass
        new_instance.save()
